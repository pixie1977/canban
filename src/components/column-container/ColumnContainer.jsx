...
const updateSimpleColumn = (columns, setColumns) => {
    return (columnId, data) => {
        setColumns(Object.entries(columns).map(([cId, col]) => {
...
const renderColumns = (columns, isAuthorized, setColumns, setViewSimpleCard) => {
...
export const ColumnContainer = ({columnsDescription, isAuthorized}) => {
    ...
    return (
        ...
        >
            {(viewSimpleCard)
                ? <BigText data={viewSimpleCard} handleClose={() => setViewSimpleCard(null)}/>
                : isAuthorized
